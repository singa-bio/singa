...
import de.bioforscher.singa.simulation.model.parameters.EnvironmentalParameterDefaults;
...
public final class DiffusionUtilities {
    ...
    public static Quantity<Diffusivity> calculateWilkeCorrelation(ChemicalEntity chemicalEntity) {
        ...
        final double dividend = WILKE_DIFFUSION_COEFFICIENT_CONSTANT.getValue().doubleValue()
                * Math.pow(EnvironmentalParameterDefaults.WATER.getMolarMass().getValue().doubleValue() * WILKE_ASSOCIATION_WATER.getValue()
                .doubleValue(), 0.5)
                * EnvironmentalParameters.getInstance().getSystemTemperature().getValue().doubleValue();
        // b = n * M(Sp)^0.6
