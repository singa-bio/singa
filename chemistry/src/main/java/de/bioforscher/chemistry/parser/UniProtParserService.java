...
public class UniProtParserService extends AbstractXMLParser {
    ...
    public void fetchResource() {
        ...
        // parse xml
        try {
            this.getXmlReader().parse(new InputSource(new ByteArrayInputStream(getFetchResult().getContent().getBytes("utf-8"))));
    ...
    public static void main(String[] args) {
        ...
        System.out.println(entity.getSourceOrganism().getName());
        System.out.println("TaxID: "+entity.getSourceOrganism().getIdentifier());
        System.out.println(entity.getSourceOrganism().getLineage().stream()
                                 .map(Taxon::getName).collect(Collectors.joining(" - " + "")));
        System.out.println();
