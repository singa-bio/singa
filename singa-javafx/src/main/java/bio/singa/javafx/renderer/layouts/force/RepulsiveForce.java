...
public class RepulsiveForce<NodeType extends Node<NodeType, Vector2D, ?>> extends Force<NodeType> {
    ...
    public RepulsiveForce(ForceDirectedGraphLayout<NodeType, ?, ?, ?> parentLayout) {
        this.parentLayout = parentLayout;
        forcePredicate = (first, second) -> true;
    }
    ...
    public void determineDisplacement(NodeType first, NodeType second) {
        ...
        parentLayout.getVelocities().put(first, firstVelocity.add(acceleration));
        // subtract second
        Vector2D secondVelocity = parentLayout.getVelocities().get(second);
        if (secondVelocity == null) {
            secondVelocity = new Vector2D();
        }
        parentLayout.getVelocities().put(second, secondVelocity.subtract(acceleration));
    }
    ...
    public Vector2D calculateAcceleration(NodeType first, NodeType second) {
        ...
        // v = unit(d) * force(m)
        return distance.normalize().multiply((parentLayout.getForceConstant() * parentLayout.getForceConstant()) / magnitude);
    }
