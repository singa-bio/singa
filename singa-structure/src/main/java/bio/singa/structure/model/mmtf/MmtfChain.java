...
public class MmtfChain implements Chain {
    ...
    public List<MmtfLeafSubstructure> getAllLeafSubstructures() {
        ...
        for (Integer relevantGroup : relevantGroups) {
            if (cachedLeaves.containsKey(relevantGroup)) {
                ...
                MmtfLeafSubstructure leaf = MmtfLeafSubstructureFactory.createLeaf(data, bytes, leafIdentifiers.get(relevantGroup), relevantGroup, atomRange.getLowerBound(), atomRange.getUpperBound());
    ...
    public Optional<MmtfLeafSubstructure> getLeafSubstructure(LeafIdentifier leafIdentifier) {
        ...
        if (cachedLeaves.containsKey(internalIndex)) {
            ...
            MmtfLeafSubstructure leaf = MmtfLeafSubstructureFactory.createLeaf(data, bytes, leafIdentifiers.get(internalIndex), internalIndex, atomRange.getLowerBound(), atomRange.getUpperBound());
