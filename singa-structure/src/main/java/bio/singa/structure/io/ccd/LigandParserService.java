...
public class LigandParserService {
    ...
    public static LeafSkeleton parseLeafSkeleton(String ligandId, LocalCcdRepository localCcdRepository) {
        logger.debug("parsing structure {} using the supplied atoms", ligandId);
        try {
            return parseLeafSkeleton(Files.newInputStream(localCcdRepository.getPathForLigandIdentifier(ligandId)));
        } catch (IOException e) {
            logger.warn("Could not parse cif file for ligand {}. If this exception" +
