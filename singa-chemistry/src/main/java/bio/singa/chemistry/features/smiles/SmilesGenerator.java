...
//        }
    ...
    private void canon() {
        ...
        // (1) calculate initial variants
        calculateInvariants();
        // (3) calculate ranks
        invariantToRanks(new ArrayList<>(moleculeGraph.getNodes()));
        // store initial invariants ordering for breaking ties
        List<MoleculeAtom> initialAtomSorting = currentRanks.entrySet().stream()
                .sorted(Map.Entry.comparingByValue())
                .map(Map.Entry::getKey)
                .collect(Collectors.toList());
        if (new HashSet<>(currentRanks.values()).size() != moleculeGraph.getNodes().size()) {
            logger.info("ranking not invariant");
            // (2) set rank based on product of neighboring primes
            for (MoleculeAtom moleculeAtom : currentRanks.entrySet().stream()
                    .sorted(Map.Entry.comparingByValue())
                    .map(Map.Entry::getKey).collect(Collectors.toList())) {
                int invariant = 1;
                for (MoleculeAtom neighbour : moleculeAtom.getNeighbours()) {
                    invariant *= primeList.get(currentRanks.get(neighbour) - 1);
                }
                atomInvariants.put(moleculeAtom, invariant);
            }
            invariantToRanks(initialAtomSorting);
        }

        // (7) check for ties
        if (currentRanks.values().stream()
                .mapToInt(Integer::intValue)
                .max()
                .getAsInt() < moleculeGraph.getNodes().size()) {
            logger.info("ties detected, breaking them");
            ...
            List<Integer> smallestDuplicateRank = rankCount.entrySet().stream()
                    .filter(rankOccurrence -> rankOccurrence.getValue() > 1)
                    .map(Map.Entry::getKey)
                    .collect(Collectors.toList());
            for (Map.Entry<MoleculeAtom, Integer> rankEntry : currentRanks.entrySet()) {
                atomInvariants.put(rankEntry.getKey(), rankEntry.getValue() * 2);
            }
            for (Integer rankdup : smallestDuplicateRank) {
                for (MoleculeAtom moleculeAtom : initialAtomSorting) {
                    if (currentRanks.get(moleculeAtom).equals(rankdup)) {
                        atomInvariants.put(moleculeAtom, atomInvariants.get(moleculeAtom) - 1);
                        break;
                    }
                }
...
//                    atomInvariants.put(atomRank.getKey(), newInvariant);
//                    invariantToRanks(invariantToRanks(););
//                    break;
//                }
//            }
//        }
//    }
    ...
    private void calculateInvariants() {
        atomInvariants = new LinkedHashMap<>();
    ...
    private void invariantToRanks(List<MoleculeAtom> sortedAtoms) {
        // first iteration, sort by initial invariant
        currentRanks = new TreeMap<>(Comparator.comparing(MoleculeAtom::getIdentifier));
