...
public abstract class FeatureProvider {
    ...
    /**
     * The Feature, that is provided by this provider.
     */
    private FeatureKind providedFeature;
    ...
    public <FeaturableType extends Featureable> void annotate(FeaturableType featureable) {
        if (!featureable.hasFeature(getProvidedFeature())) {
            if (isAvailableFor(featureable)) {
                resolveRequirements(featureable);
                featureable.assignFeature(getFeatureFor(featureable));
            } else {
                throw new IllegalArgumentException("The Feature "+getProvidedFeature()+" is not available for " + featureable.getClass().getSimpleName());
            }
        }
    }
    ...
    protected void setProvidedFeature(FeatureKind providedFeature) {
        this.providedFeature = providedFeature;
    }

    public FeatureKind getProvidedFeature() {
